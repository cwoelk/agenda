...
Postgres.prototype.saveJob = function(job, lastModifiedBy, cb) {
  ...
  this._connect(function(error, client) {
    ...
    var query = squel.select().from(self._table).where(whereExpr.toString()).toString();
...
Postgres.prototype._updateJob = function(client, props, cb) {
  ...
  if (props.nextRunAt)  {
    count++; query = query.set("nextrunat", Math.floor(props.nextRunAt.getTime()/ 1000));
  }
  if (props.lockedAt)  {
    count++; query = query.set("lockedat", Math.floor(props.lockedAt.getTime()/ 1000));
  }
  if (props.lastModifiedBy)  {
    count++; query = query.set("lastmodifiedby", props.lastModifiedBy);
  }
  if (props.repeatInterval)  {
    count++; query = query.set("repeatinterval", props.repeatInterval);
  }
  if (props.repeatTimezone)  {
...
function sanitizeJob(baseJob) {
  ...
  if (baseJob.lockedat) job.lockedAt = new Date(baseJob.lockedat*1000);
  if (baseJob.nextrunat) job.nextRunAt = new Date(baseJob.nextrunat*1000);
