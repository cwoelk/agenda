...
Postgres.prototype.saveJob = function(job, lastModifiedBy, cb) {
  ...
  this._connect(function(error, client) {
    ...
    if (id) {
      whereExpr = whereExpr.and('id = ?', props.id);
    } else if (props.type == 'single') {
      whereExpr = whereExpr.and('id = ?', props.id).and('type = ?', props.type);
    } else if (unique) {
    ...
    client.query(query, function(error, result) {
      ...
      if (result.rowCount  > 0) {
        self._updateJob(client, props, cb);
      } else {
        self._insertJob(client, props, cb);
      }
    });
  })
};
...
Postgres.prototype._insertJob = function(client, props, cb) {
  var nextRunAt = props.nextRunAt ? props.nextRunAt.toString() : null;
  var lockedAt = props.lockedAt ? props.lockedAt.toString() : null;
  var query =squel.insert()
    .into(this._table)
    .set("name", props.name)
    .set("disabled", props.disabled || false)
    .set("data", JSON.stringify(props.data) || null)
    .set("priority", props.priority || null)
...
Postgres.prototype._updateJob = function(client, props, cb) {
  ...
  if (props.name)  count++; query = query.set("name", props.name);
  if (props.disabled)  count++; query = query.set("disabled", props.disabled);
  if (props.data)  count++; query = query.set("data", JSON.stringify(props.data));
  if (props.priority)  count++; query = query.set("priority", props.priority);
  if (props.nextRunAt)  count++; query = query.set("nextrunat", props.nextRunAt);
  if (props.lockedAt)  count++; query = query.set("lockedat", props.lockedAt);
  if (props.lastModifiedBy)  count++; query = query.set("lastmodifiedby", props.lastModifiedBy);
  if (props.repeatInterval)  count++; query = query.set("repeatinterval", props.repeatInterval);
  if (props.repeatTimezone)  count++; query = query.set("repeattimezone", props.repeatTimezone);
...
Postgres.prototype._findAndLockNextJob = function(jobName, definition, nextScanAt, cb) {
  console.log('_findAndLockNextJob');
};
...
Postgres.prototype._unlockJobs = function(lockedJobIds, done) {
  console.log('_unlockJobs');
};

Postgres.prototype.findUnlockedJob = function(job, cb) {
  console.log('findUnlockedJob');
};
...
Postgres.prototype._connect = function(cb) {
  ...
  } else if (mdb instanceof Client) {
    this._mdb.connect(function(error) {
      if (cb) cb(error, this._mdb);
    });
  } else {
...
function createTables(client, table, cb) {
  ...
  query = query.concat('nextRunAt VARCHAR(1024),');
  query = query.concat('lockedAt VARCHAR(1024),');
