...
function getDbAdapterClass(config, cb) {
  ...
  for (var i=0; i < keys.length; i++) {
    ...
    if (Connector) {
      return new Connector(config, cb);
    }
  }
  return new Mongo(config, cb);
}
...
Agenda.prototype.pg = function( mdb, table, cb ) {
  var Connector = connectorMap.pg;
  this._dbAdapter = new Connector();
  this._dbAdapter.pg(mdb, table, cb);
  return this
}
...
// TODO: Refactor cancel arguments
Agenda.prototype.purge = function(cb) {
  var definedNames = Object.keys(this._definitions);

  if (!(this._dbAdapter instanceof Mongo)) {
    this.cancel(getPurgeQuery(definedNames), cb);
  } else {
...
function getPurgeQuery(definedNames) {
  if (definedNames && definedNames.length) {
    var query = 'name NOT IN (';

    definedNames.forEach(function(definedName, index) {
      query = query.concat(definedName);
      if (index < definedNames.length - 1) {
        query = query.concat(',')
      }
    });
