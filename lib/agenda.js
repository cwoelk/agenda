...
var Agenda = module.exports = function(config, cb) {
  ...
  this._dbAdapter = getDbAdapterClass(config, cb || baseCallback);
};
...
function baseCallback( err, result ){
  if (err) {
    self.emit('error', err);
  } else {
    self.emit('ready');
...
Agenda.prototype.mongo = function( mdb, collection, cb ) {
  this._dbAdapter.mongo(mdb, collection, function( err, collection ){
    if (err) {
      self.emit('error', err);
    } else {
      self.emit('ready');
    }
    if (cb) cb(err, collection);
  });
  return this
}

Agenda.prototype.pg = function( mdb, table, cb ) {
  this._dbAdapter.pg(mdb, table, function( err ){
    if (err) {
      self.emit('error', err);
    } else {
      self.emit('ready');
...
Agenda.prototype.db_init = function( collection, cb ) {
  this._dbAdapter.db_init(collection, function( err, collection ){
    if (err) {
      self.emit('error', err);
    } else {
      self.emit('ready');
